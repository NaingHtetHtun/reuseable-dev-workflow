...
## Phase 7b — Workflow Triggers

**Status**: ✅ COMPLETED

**Feature plan**: `docs/features/008-workflow-triggers.md`

**Goal**: Establish a reusable trigger system that defines how and when workflows start executing.

**Why it exists**: Workflows currently can only be triggered manually via API. Real-world workflows need to respond to external events, schedules, and API calls automatically.

**Major features**:

- Trigger abstraction (framework-independent interface)
- Trigger type registry
- Manual trigger (always available)
- Webhook trigger (HTTP endpoint)
- Scheduled trigger (cron definition)
- Trigger validation and lifecycle
- HMAC webhook security
- Idempotency for webhook deliveries
- Trigger → workflow execution integration
- API endpoints for trigger management

**Dependencies**: Phase 3 (workflows), Phase 5 (credentials).

**Expected deliverables**:

- `packages/workflow-core/src/trigger-system/` — Framework-independent trigger system
- `apps/api/src/modules/triggers/` — Trigger management module
- `apps/api/src/modules/webhooks/` — Webhook execution module
- Manual, webhook, and scheduled trigger implementations
- Feature documentation

**Testing requirements**:

- Unit tests for trigger types and registry
- Unit tests for webhook HMAC validation
...
- [x] Manual, webhook, and scheduled triggers implemented
- [x] Webhook HMAC validation works
- [x] Idempotency works
- [x] API endpoints work
- [x] All tests pass (354 total)

**Intentionally out of scope**:

- Visual workflow builder
- Background worker/queue system
- Full cron scheduler infrastructure
