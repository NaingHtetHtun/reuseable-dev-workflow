...
let OAuthService = OAuthService_1 = class OAuthService {
    ...
    async generateAuthorizationUrl(params) {
        ...
        this.logger.log(`Generated authorization URL for ${params.providerType} (project: ${params.projectId})`);
        return {
    ...
    async handleCallback(params) {
        ...
        this.logger.log(`OAuth callback handled for ${params.providerType} (credential: ${credentialId})`);
        return {
            credentialId,
    ...
    async refreshToken(params) {
        ...
        if (!tokenManager) {
            tokenManager = new workflow_core_1.OAuthTokenManager(provider, async (id) => {
        ...
        const accessToken = await tokenManager.getValidAccessToken(params.credentialId, params.clientId, params.clientSecret, credential.metadata ?? {});
        return { accessToken };
    }
    getProviders() {
