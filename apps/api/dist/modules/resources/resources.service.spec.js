...
describe('ResourcesService', () => {
    ...
    describe('findOne', () => {
        ...
        it('should throw NotFoundException for missing resource', async () => {
            prisma.resource.findFirst.mockResolvedValueOnce(null);
            await expect(service.findOne('project-1', 'nonexistent')).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('update', () => {
        ...
        it('should throw NotFoundException for missing resource', async () => {
            prisma.resource.findFirst.mockResolvedValueOnce(null);
            await expect(service.update('project-1', 'nonexistent', { displayName: 'Test' })).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('remove', () => {
        ...
        it('should throw NotFoundException for missing resource', async () => {
            prisma.resource.findFirst.mockResolvedValueOnce(null);
            await expect(service.remove('project-1', 'nonexistent')).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('createVersion', () => {
        ...
        it('should throw NotFoundException for missing resource', async () => {
            prisma.resource.findFirst.mockResolvedValueOnce(null);
            await expect(service.createVersion('project-1', 'nonexistent', '1.0.0')).rejects.toThrow(common_1.NotFoundException);
        });
        it('should throw BadRequestException for invalid version', async () => {
            await expect(service.createVersion('project-1', 'res-1', 'invalid')).rejects.toThrow(common_1.BadRequestException);
        });
        it('should throw ConflictException for duplicate version', async () => {
            ...
            await expect(service.createVersion('project-1', 'res-1', '1.0.0')).rejects.toThrow(common_1.ConflictException);
        });
    });
    ...
    describe('generatePrisma', () => {
        ...
        it('should throw NotFoundException for missing resource', async () => {
            prisma.resource.findFirst.mockResolvedValueOnce(null);
            await expect(service.generatePrisma('project-1', 'nonexistent')).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('generateValidation', () => {
        ...
        it('should throw NotFoundException for missing resource', async () => {
            prisma.resource.findFirst.mockResolvedValueOnce(null);
            await expect(service.generateValidation('project-1', 'nonexistent', 'create')).rejects.toThrow(common_1.NotFoundException);
        });
    });
});
//# sourceMappingURL=resources.service.spec.js.map