...
import { INTERFACE_TEMPLATE, CREATE_INPUT_TEMPLATE, UPDATE_INPUT_TEMPLATE, FILE_HEADER_TEMPLATE, } from './templates';
/**
...
export class TypeScriptAdapter {
    ...
    compile(definition, options) {
        ...
        for (const resource of definition.resources) {
            try {
                ...
                errors.push(`Failed to generate code for resource "${resource.name}": ${err instanceof Error ? err.message : String(err)}`);
            }
        }
    ...
    generateResourceFiles(resource, options, prefix) {
        ...
        const header = options.includeComments
            ...
            : '';
        const content = header + interfaceContent + '\n\n' + createInputContent + '\n\n' + updateInputContent;
        files.push({
            path: filePath,
            content,
