...
export class ResourceValidator {
  ...
  validateField(field) {
    ...
    if (!field.name || typeof field.name !== 'string') {
      ...
      errors.push(
        `Field name "${field.name}" must be snake_case (lowercase letters, numbers, underscores)`,
      );
    }
    ...
    if (!field.type || !VALID_FIELD_TYPES.includes(field.type)) {
      errors.push(
        `Field "${field.name}" has invalid type "${field.type}". Valid types: ${VALID_FIELD_TYPES.join(', ')}`,
      );
    }
  ...
  validateFieldConstraints(field) {
    ...
    if (field.type === 'relation') {
      ...
      if (
        ...
        errors.push(
          `Relation field "${field.name}" has invalid relationType "${field.relationType}"`,
        );
      }
    }
    ...
    if (field.type === 'string' || field.type === 'text') {
      if (field.minLength !== undefined && field.maxLength !== undefined) {
        if (field.minLength > field.maxLength) {
          errors.push(
            `Field "${field.name}": minLength (${field.minLength}) must be <= maxLength (${field.maxLength})`,
          );
        }
      }
    }
    ...
    if (field.type === 'integer' || field.type === 'float') {
      if (field.minimum !== undefined && field.maximum !== undefined) {
        if (field.minimum > field.maximum) {
          errors.push(
            `Field "${field.name}": minimum (${field.minimum}) must be <= maximum (${field.maximum})`,
          );
        }
      }
    }
