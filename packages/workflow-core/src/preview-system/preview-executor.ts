...
export class PreviewExecutor {
  ...
  private dryRun(
    ...
    if (options.maxNodes && request.definition.nodes.length > options.maxNodes) {
      warnings.push(
        `Only ${options.maxNodes} of ${request.definition.nodes.length} nodes included (maxNodes limit)`,
      );
    }
  ...
  private async executeWorkflow(
    ...
    if (options.maxNodes && sortedNodes.length > options.maxNodes) {
      warnings.push(
        `Only ${options.maxNodes} of ${sortedNodes.length} nodes executed (maxNodes limit)`,
      );
    }
  ...
  private getNodeInput(node: { id: string }, context: ExecutionContext): unknown {
  ...
  private topologicalSort(definition: WorkflowDefinition): WorkflowDefinition['nodes'] {
    ...
    const sorted: WorkflowDefinition['nodes'] = [];
